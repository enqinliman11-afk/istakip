...
export default function TeamLeadCalendarPage() {
    const { tasks, getCategoryById, getClientById, getTaskAssignees } = useData();
    const [selectedDate, setSelectedDate] = useState<Date | null>(null);
    const [currentMonth, setCurrentMonth] = useState(new Date());

    // Sadece AKTİF işler (tamamlanan hariç)
    const activeTasks = useMemo(() => {
        return tasks.filter(t => t.status !== 'TAMAMLANDI');
    }, [tasks]);
        ...
        <>
            <header className="page-header">
                <div className="page-header-content">
                    <h1 className="page-title">📅 İş Takvimi</h1>
                    <p style={{ fontSize: '0.875rem', color: 'var(--text-muted)' }}>
                        Tüm aktif işlerin deadline'larını görüntüleyin
                    </p>
                </div>
            </header>
                        ...
                        return (
                            <div key={task.id} style={{ padding: '1rem', background: 'var(--bg-secondary)', borderRadius: '0.5rem', borderLeft: '4px solid var(--primary-500)' }}>
                                ...
                                {(task.startTime && task.endTime) && (
                                    <div style={{ padding: '0.5rem', background: 'var(--bg-tertiary)', borderRadius: '0.5rem', marginBottom: '0.5rem' }}>
                                        <span>⏱️ Çalışma Süresi: {workDuration}</span>
                                    </div>
                                )}
                                {assignees.length > 0 && (
                                    <div style={{ display: 'flex', gap: '0.25rem', flexWrap: 'wrap' }}>
                                        {assignees.map(u => (
                                            <span key={u.id} style={{ padding: '0.25rem 0.5rem', background: 'var(--bg-tertiary)', borderRadius: '0.25rem', fontSize: '0.75rem', color: 'var(--text-secondary)' }}>{u.name}</span>
                                        ))}
                                    </div>
                                )}
                            </div>
